...
describe('From tenji', () => {
  describe('Basic tenji', () => {
    describe('Hiragana', () => {
      ...
      // https://github.com/uhyo/tenji/issues/2
      it('記号 and 清音', () => {
        expect(fromTenji('⠁⠃⠉⠰⠀⠡⠣⠩⠲')).toBe('あいう、かきく。');
      });
    });
    it('Number', () => {
    ...
    it('Alphabets', () => {
      ...
      expect(fromTenji('⠰⠠⠠⠥⠎⠁')).toBe('USA');
      expect(fromTenji('⠰⠠⠁⠤⠈⠩⠉')).toBe('Aきゅう');
      expect(fromTenji('⠰⠠⠃⠼⠛⠓⠛')).toBe('B787');
  ...
  describe('漢点字', () => {
    ...
    it('kanji and others', () => {
      expect(fromTenji('⠷⣸⠓⢜⡠⢠⠀⢠⢀⣂⡲⠔⠒⡢⠲⠢⣀⠗⢺⡷⢌⢤', { kanji: true })).toBe(
        '点字を、Unicode変換。',
      );
    });
    ...
    it('Spacieal わ行 characters', () => {
      expect(fromTenji('⡀⢐⠆⠒⢐⠖⡠', { kanji: true })).toBe('わゐうゑを');
    });
    it('Special small か, け', () => {
      expect(fromTenji('⣰⠒⡤⢐⢖⡄⠫⡤⢐⢂⡄', { kanji: true })).toBe('3ヶ月ヵ');
    });
    it('Katakana', () => {
